...
function MarioGame() {
  ...
  this.showInstructions = function() {
    gameCanvas.writeText('Controls: Arrow keys for direction, shift to run, ctrl for bullets', 30, 30);
  ...
  this.checkEnemyMarioCollision = function() {
    for (let i = 0; i < goombas.length; i++) {
      if (!mario.invulnerable && goombas[i].state != 'dead' && goombas[i].state != 'deadFromBullet') {
        ...
        let collWithMario = that.collisionCheck(goombas[i], mario);
    ...
    for (let i = 0; i < goombas.length; i++) {
      for (let j = 0; j < bullets.length; j++) {
        if (goombas[i] && goombas[i].state != 'dead') {
          ...
          let collWithBullet = that.collisionCheck(goombas[i], bullets[j]);
        }
